...
.content
  ...
  .widget.tw-h-full(v-else)
    VideoCard(:ref="item.name" :camera="item" title titlePosition="top" status :stream="item.live" :refreshSnapshot="!item.live" :notifications="Boolean(item.lastNotification)" hideController)
</template>
